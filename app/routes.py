...
def extract():
    if request.method== "POST":
        ...
        opinions = pd.read_json(json.dump(all_opinions, jf, indent=4, ensure_ascii=False))
        ...
        recommendation.plot.pie(label="", 
                                autopct="%1.1f%%",
                                labels=["Nie polecam","Polceam","Nie mam zdania"],
...
@app.route('/products')
def products():
    #lista kod produktów o ktorych są pobrane opinie
    #pobranie z plików JSON ze statystykami danych do  listy produktów
    #rzekazanie listy prodkutów od widoków
